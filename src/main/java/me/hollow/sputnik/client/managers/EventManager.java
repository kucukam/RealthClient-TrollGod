...
public class EventManager implements IListener {
    ...
    public void init() {
        ...
        Sputnik.INSTANCE.getBus().register(this);
        size = Sputnik.INSTANCE.getModuleManager().getModules().size();
    }
    ...
    public void onUpdate(final LivingEvent.LivingUpdateEvent event) {
        if (event.getEntityLiving() == Minecraft.getMinecraft().player) {
            for (int i = 0; i < size; i++) {
                final Module module = Sputnik.INSTANCE.getModuleManager().getModules().get(i);
                if (module.isEnabled()) {
                    module.onUpdate();
                }
            }
            Sputnik.INSTANCE.getSafeManager().update();
            Sputnik.INSTANCE.getPopManager().update();
        }
    }
