                    ...
                    if (!Strings.isNullOrEmpty(data.getProfile().getName()) || data.getProfile().getId() != null) {
                        ...
                        switch (packet.getAction()) {
                            case ADD_PLAYER:
                                ...
                                Sputnik.INSTANCE.getBus().post(new ConnectionEvent(0, id, name));
                                break;
                            case REMOVE_PLAYER:
                                EntityPlayer entity = mc.world.getPlayerEntityByUUID(id);
                                if (entity != null) {
                                    String logoutName = entity.getName();
                                    Sputnik.INSTANCE.getBus().post(new ConnectionEvent(1, entity, id, logoutName));
                                } else {
                                    Sputnik.INSTANCE.getBus().post( new ConnectionEvent(2, id, null));
                                }
                                break;
                        }
                    }
