...
public class ConfigManager {
    ...
    public void loadConfig(String name) {
        List<File> files = Arrays.stream(Objects.requireNonNull(new File("TrollGod").listFiles()))
                .filter(File::isDirectory)
                .collect(Collectors.toList());
        if (files.contains(new File("TrollGod/" + "modules" + "/"))) {
            config = "TrollGod/" + "modules" + "/";
        } else {
            config = "TrollGod/modules/";
        }
    ...
    public void init() {
        features.addAll(Sputnik.INSTANCE.getModuleManager().getModules());
