        ...
        for (int i = 0; i < size; i++) {
            ...
            if (player != null && player != mc.player && player.isEntityAlive() && !player.isPlayerSleeping()) {
                if (rotationList.get(player) != null && mc.player.getDistanceSq(player) < 2500) {
                    renderSkeleton(player, rotationList.get(player), Sputnik.INSTANCE.getFriendManager().isFriend(player) ? new Color(0xFF55C0ED) : new Color(Colours.INSTANCE.getColor()));
                }
            }
        }
    }
