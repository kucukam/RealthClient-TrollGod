    ...
    public void run(int mouse) {
        if (mouse == 2 && friends.getValue()) {
            if (mc.objectMouseOver.entityHit != null) {
                ...
                if (Sputnik.INSTANCE.getFriendManager().isFriend(entity.getName())) {
                    Sputnik.INSTANCE.getFriendManager().removeFriend(entity.getName());
                } else {
                    Sputnik.INSTANCE.getFriendManager().addFriend(entity.getName());
                }
            }
        }
    }
