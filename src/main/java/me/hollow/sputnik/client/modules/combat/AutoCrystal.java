...
public final class AutoCrystal extends Module {
    ...
    private final Setting<Boolean> autoSwitch = register(new Setting<>("Auto Switch", true, v -> page.getValue() == Page.PLACE));
    ...
    public void onTick() {
        ...
        if (!offhand && mc.player.getHeldItemMainhand().getItem() != Items.END_CRYSTAL && !autoSwitch.getValue())
            return;
        if (!Sputnik.INSTANCE.getEventManager().switchTimerPassed(cooldown.getValue())) {
            return;
        }
