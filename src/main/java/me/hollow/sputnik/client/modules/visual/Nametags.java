...
public final class Nametags extends Module {
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        if(rect.getValue()) {
            if (gradientBorder.getValue()) {
                ...
                RenderUtil.drawBorderedRect(-width - 2, -(mc.fontRenderer.FONT_HEIGHT + 1), width + 2F, 1.5F, 0x55000000, rectBorder.getValue() ? (Sputnik.INSTANCE.getFriendManager().isFriend(player) ? 0xFF55C0ED : Colours.INSTANCE.getColor()) : 0x00000000);
            }
        }
    ...
    private String getDisplayTag(EntityPlayer player) {
        ...
        if (totemPops.getValue()) {
            final Map<String, Integer> registry = Sputnik.INSTANCE.getPopManager().getPopMap();
    ...
    private int getDisplayColour(EntityPlayer player) {
        if (Sputnik.INSTANCE.getFriendManager().isFriend(player)) {
            return 0xFF55C0ED;
