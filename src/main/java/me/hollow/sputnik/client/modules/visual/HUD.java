...
public final class HUD extends Module {
    ...
    @Override
    public void onLoad() {
        modules = new ArrayList<>(Sputnik.INSTANCE.getModuleManager().getModules());
    }
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        modules.sort(Comparator.comparingDouble(m -> -Sputnik.fontManager.getStringWidth(m.getLabel() + m.getSuffix())));
    }
    ...
    public final void onRender2D() {
        ...
        if (renderWatermark.getValue()) {
            Sputnik.fontManager.drawString(customWatermark.getValue() ? watermarkString.getValue() : "TrollGod.CC v" + Sputnik.VERSION, 2, offsetWatermark.getValue() ? 10 : 2, Colours.INSTANCE.getColor());
        }
        ...
        if (armorHud.getValue()) {
            ...
            for (int j = 0; j < armorInvSize; ++j) {
                ...
                Sputnik.fontManager.drawString(dmg + "", x + 8 - (mc.fontRenderer.getStringWidth(dmg + "") >> 1), y + (renderArmor.getValue() ? -8 : 6), is.getItem().getRGBDurabilityForDisplay(is));
            }
            GlStateManager.enableDepth();
            GlStateManager.disableLighting();
        }
        ...
        if (coords.getValue()) {
            Sputnik.fontManager.drawString("XYZ \u00a7f" + (int) mc.player.posX + ", " + (int) mc.player.posY + ", " + (int) mc.player.posZ,2, resolution.getScaledHeight() - 10, Colours.INSTANCE.getColor());
        }
        ...
        if (direction.getValue()) {
            ...
            Sputnik.fontManager.drawString(facing, 2, resolution.getScaledHeight() - 10 - (coords.getValue() ? 10 : 0), Colours.INSTANCE.getColor());
        }

        int daFunnies = 0;
        if (kmh.getValue()) {
            final String kmhString = "Speed \u00a7f" + MathUtil.getSpeedInKMH() + "km/h";
            Sputnik.fontManager.drawString(kmhString, resolution.getScaledWidth() - Sputnik.fontManager.getStringWidth(kmhString) - 2, resolution.getScaledHeight() - 10, Colours.INSTANCE.getColor());
            daFunnies += 10;
        }

        if (tps.getValue()) {
            final String tpsString = "TPS \u00a7f" + String.format("%.1f", Sputnik.INSTANCE.getTpsManager().getTPS());
            Sputnik.fontManager.drawString(tpsString, resolution.getScaledWidth() - Sputnik.fontManager.getStringWidth(tpsString) - 2, resolution.getScaledHeight() - 10 - daFunnies, Colours.INSTANCE.getColor());
            daFunnies += 10;
        }

        if (fps.getValue()) {
            final String fpsString = "FPS \u00a7f" + mc.getDebugFPS();
            Sputnik.fontManager.drawString(fpsString, resolution.getScaledWidth() - Sputnik.fontManager.getStringWidth(fpsString) - 2, resolution.getScaledHeight() - 10 - daFunnies, Colours.INSTANCE.getColor());
            daFunnies +=10;
        }
        ...
        if (ping.getValue()) {
            final String pingString = "Ping \u00a7f" + Sputnik.INSTANCE.getTpsManager().getPing();
            Sputnik.fontManager.drawString(pingString, resolution.getScaledWidth() - Sputnik.fontManager.getStringWidth(pingString) - 2, resolution.getScaledHeight() - 10 - daFunnies, Colours.INSTANCE.getColor());
        }
        ...
        if (arrayList.getValue()) {
            ...
            for (final Module module : modules) {
                if (module.isEnabled() && module.drawn.getValue()) {
                    ...
                    Sputnik.fontManager.drawString(fullLabel, resolution.getScaledWidth() - Sputnik.fontManager.getStringWidth(fullLabel) - 2, offset += 10, mode.getValue() == ColorMode.SYNC ? Colours.INSTANCE.getColor() : module.getColor());
                }
            }
        }
    }
