...
public class BindButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isListening) {
            Sputnik.fontManager.drawString("Listening..", x + 2, y - 1 - TrollGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        } else {
            Sputnik.fontManager.drawString(setting.getName() + " " + ChatFormatting.GRAY + setting.getValue().toString(), x + 2.3F, y - 1.7F - TrollGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        }
    }
