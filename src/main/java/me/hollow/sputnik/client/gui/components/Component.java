...
public class Component implements Minecraftable {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(x - 4, y - 2, x + width + 4, y + height - 6, new Color(ClickGui.getInstance().categoryRed.getValue(), ClickGui.getInstance().categoryGreen.getValue(), ClickGui.getInstance().categoryBlue.getValue(), ClickGui.getInstance().categoryAlpha.getValue()).getRGB());
        if (open) {
            RenderUtil.drawRect(x, y + 12F, x + width, y + height + totalItemHeight, new Color(0, 0, 0, ClickGui.getInstance().alpha.getValue()).getRGB());
        }
        Sputnik.fontManager.drawString(getName(), x + (float)width / 2 - Sputnik.fontManager.getStringWidth(getName()) / 2, y - 4F - TrollGui.getClickGui().getTextOffset(), 0xFFFFFF);
        if (open) {
