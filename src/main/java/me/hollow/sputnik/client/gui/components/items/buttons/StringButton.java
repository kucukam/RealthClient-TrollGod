...
public class StringButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if(isListening) {
            Sputnik.fontManager.drawString(currentString.getString() + "_", x + 2F, y - 1F - TrollGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        } else {
            Sputnik.fontManager.drawString((setting.getName().equals("Buttons") ? "Buttons " : (setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + setting.getValue(), x + 2F, y - 1F - TrollGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        }
    }
    ...
    public void setString(String newString) {
        Sputnik.INSTANCE.getBus().post(new ClientEvent(null));
